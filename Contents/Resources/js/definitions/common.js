...
window.processLinks = (function(){
  var
    ...
    SPOTURI = "spotify:(track|album|artist|search|playlist|user|radio):[^<>]+",
    
    MAILTO = "mailto:",
    EMAIL = "(?:" + MAILTO + ")?[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@" + HOST_OR_IP + QUERY_FRAG + "(?!\\w)",
    
    URI_RE = new RegExp( "(?:" + URI1 + "|" + URI2 + "|" + EMAIL + "|" + SPOTURI + ")", "ig" ),
...
function handleEmbeds(link, text){
    ...
    if(link.match(/(mp3|m4a|wav|aiff)$/i)) {
	 ...
	 } else if(link.match(/5min\.com|flickr\.com|video\.google\.|hulu\.com|metacafe\.com|qik\.com|slideshare\.com|twitpic\.com|viddler\.com|vimeo\.com|youtube\.com\/watch/ig)) {
       result = embedURLs(link, text, true);
   } else if(link.match(/cl\.ly|penr\.ee|cloud\..*\..*/ig)) {
...
function embedPastie(link, text){   
    ...
    if (link.match(/^http:\/\/pastie\.org\/\d+\.txt$/) || link.match(/^http:\/\/pastie\.org\/\d+$/) || link.match(/^http:\/\/www\.pastie\.org\/\d+$/)) {
      ...
      console.log(data);
			var testthing =  data[9].innerText;
...
function updateProviders()
{
    ...
    function OEmbedProvider(name, urlPattern, oEmbedUrl, callbackparameter) {
        ...
        this.embedCode = function(externalUrl, text, embedCallback) {
            ...
            try {
                request.open("HEAD", requestUrl, false);
                request.send(null);
            } catch (e) {
                return externalUrl;
            }
            ...
            if (request.status == '200') {
                ...
                switch(type) {
                    ...
                    default:
                        code = createLinkFromText(externalUrl, externalUrl); break;
                }
            } else {
